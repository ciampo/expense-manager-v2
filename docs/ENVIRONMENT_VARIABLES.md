...
> **Note:** Run `npx @convex-dev/auth` (or `npx @convex-dev/auth --prod` for production) to set these. `JWT_PRIVATE_KEY` and `JWKS` are auto-generated. `SITE_URL` is prompted — enter your app's URL (used for callback links in OTP emails).
>
