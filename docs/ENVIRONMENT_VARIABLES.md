...
| ----------------- | -------- | ----------------------------------------------------------------------------------------- |
| `SITE_URL`        | Yes      | Set by `npx @convex-dev/auth` (prompted for your app's URL, e.g. `http://localhost:3000`) |
| `JWT_PRIVATE_KEY` | Yes      | Auto-generated by `npx @convex-dev/auth` — do not edit                                    |
| `JWKS`            | Yes      | Auto-generated by `npx @convex-dev/auth` — do not edit                                    |

> **Note:** `JWT_PRIVATE_KEY` and `JWKS` are auto-generated by `npx @convex-dev/auth` (or `npx @convex-dev/auth --prod` for production) — do not edit them manually. `SITE_URL` is **not** auto-configured; the command prompts you for it — enter your app's URL (used for callback links in OTP emails).
>
> **`SITE_URL` vs `CONVEX_SITE_URL`:** These are different variables.
> `SITE_URL` is a user-set env var — your app's URL (e.g. `http://localhost:3000` for dev, `https://yourapp.com` for prod).
