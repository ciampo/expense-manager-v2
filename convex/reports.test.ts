...
describe('reports.availableMonths', () => {
  ...
  it('excludes months that only contain expenses with isDraft undefined', async () => {
    const t = convexTest(schema, modules)
    const { userId, asUser } = await setupAuthenticatedUser(t)
    const categoryId = await setupCategory(t, userId)
...
describe('reports.monthlyData', () => {
  ...
  it('filters out categories belonging to other users', async () => {
    ...
    await insertExpense(t, user1Id, otherUserCat, {
      date: '2026-03-05',
      merchant: 'Test',
      amount: 500,
    })
  ...
  it('allows predefined categories (no userId) for all users', async () => {
    ...
    await insertExpense(t, userId, predefinedCat, { date: '2026-03-05', merchant: 'Restaurant' })
  ...
  it('excludes expenses with isDraft undefined from totals', async () => {
    const t = convexTest(schema, modules)
    const { userId, asUser } = await setupAuthenticatedUser(t)
    const categoryId = await setupCategory(t, userId)
...
describe('reports.monthlyAttachments', () => {
  ...
  it('does not include expenses from other users', async () => {
    ...
    await insertExpense(t, user2Id, cat2, {
      date: '2026-03-10',
  ...
  it('excludes attachments from expenses with isDraft undefined', async () => {
    const t = convexTest(schema, modules)
    const { userId, asUser } = await setupAuthenticatedUser(t)
    const categoryId = await setupCategory(t, userId)

    const storageId = await t.run(async (ctx) => ctx.storage.store(new Blob(['legacy-receipt'])))
